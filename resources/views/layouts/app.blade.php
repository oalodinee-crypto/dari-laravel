...
<title>@yield('title', 'DARI') - نظام داري لإدارة العقارات</title>
...
<img src="{{ asset('images/logo.jpg') }}" alt="DARI" class="sidebar-logo" onerror="this.src='https://via.placeholder.com/100x60/667eea/fff?text=DARI'">
</div>
...
<div class="nav-section">{{ __('messages.system_management') }}</div>
...
<span>{{ __('messages.buildings') }}</span>
</a>
...
{{-- ===================== --}}
{{-- قائمة مالك المبنى Manager --}}
{{-- ===================== --}}
@if(auth()->user()->hasRole('Manager'))
<div class="nav-section">{{ __('messages.building_management') }}</div>
...
<span>{{ __('messages.maintenance_requests') }}</span>
</a>

<a href="{{ route('manager.unit-requests') }}" class="nav-link {{ request()->routeIs('manager.unit-requests') ? 'active' : '' }}">
...
{{-- ===================== --}}
{{-- قائمة الساكن Resident --}}
{{-- ===================== --}}
@if(auth()->user()->hasRole('Resident'))
<div class="nav-section">{{ __('messages.my_services') }}</div>

<a href="{{ route('resident.my-unit') }}" class="nav-link {{ request()->routeIs('resident.my-unit') ? 'active' : '' }}">
<i class="bi bi-door-open"></i>
<span>وحدتي</span>
</a>

<a href="{{ route('resident.my-maintenance') }}" class="nav-link {{ request()->routeIs('resident.my-maintenance') ? 'active' : '' }}">
<i class="bi bi-tools"></i>
<span>{{ __('messages.maintenance_requests') }}</span>
</a>

<a href="{{ route('resident.my-invoices') }}" class="nav-link {{ request()->routeIs('resident.my-invoices') ? 'active' : '' }}">
<i class="bi bi-receipt"></i>
<span>{{ __('messages.my_invoices') }}</span>
</a>

<a href="{{ route('resident.pay-online') }}" class="nav-link {{ request()->routeIs('resident.pay-online') ? 'active' : '' }}">
<i class="bi bi-wallet2"></i>
<span>{{ __('messages.pay_online') }}</span>
</a>

<a href="{{ route('resident.my-complaints') }}" class="nav-link {{ request()->routeIs('resident.my-complaints') ? 'active' : '' }}">
<i class="bi bi-chat-dots"></i>
<span>{{ __('messages.my_complaints') }}</span>
</a>

<a href="{{ route('resident.request-unit') }}" class="nav-link {{ request()->routeIs('resident.request-unit') ? 'active' : '' }}">
<i class="bi bi-house-add"></i>
<span>طلب وحدة</span>
</a>

<a href="{{ route('resident.my-requests') }}" class="nav-link {{ request()->routeIs('resident.my-requests') ? 'active' : '' }}">
...
{{-- ===================== --}}
{{-- التواصل - لجميع المستخدمين --}}
{{-- ===================== --}}
<div class="nav-section">{{ __('messages.communication') }}</div>
...
@endphp
@if($sidebarUnreadMsgCount > 0)
<span class="badge bg-danger rounded-pill ms-auto">{{ $sidebarUnreadMsgCount }}</span>
@endif
</a>
...
<span class="badge bg-danger rounded-pill ms-auto">{{ $sidebarUnreadNotifCount }}</span>
@endif
</a>
...
@if(auth()->user()->avatar)
<img src="{{ asset('storage/' . auth()->user()->avatar) }}" class="rounded-circle" style="width:40px;height:40px;object-fit:cover;cursor:pointer;" onclick="window.location='{{ route('profile') }}'">
@else
<div class="avatar-circle" onclick="window.location='{{ route('profile') }}'">{{ mb_substr(auth()->user()->name ?? 'م', 0, 1) }}</div>
@endif
<span class="user-name-text d-none d-md-inline" style="font-size: 0.95rem;">{{ __('messages.welcome') }} {{ auth()->user()->name ?? '' }}</span>
...
<div class="d-flex align-items-center gap-2">
    <!-- تغيير اللغة -->
    <a href="{{ route('lang.switch', app()->getLocale() == 'ar' ? 'en' : 'ar') }}" class="top-bar-icon" title="{{ app()->getLocale() == 'ar' ? 'English' : 'العربية' }}">
    ...
    <div class="position-relative notification-wrapper">
        ...
        <span class="notif-badge notif-badge-red">{{ $unreadNotifCount > 99 ? '99+' : $unreadNotifCount }}</span>
        @endif
    ...
    <div class="position-relative message-wrapper">
        ...
        @if($unreadMsgCount > 0)
        <span class="notif-badge notif-badge-blue">{{ $unreadMsgCount > 99 ? '99+' : $unreadMsgCount }}</span>
        @endif
        ...
        <div class="dropdown-menu-custom messages-dropdown" id="messagesDropdown">
            ...
            <div class="dropdown-footer">
                <a href="{{ route('messages.index') }}">عرض جميع الرسائل</a>
            </div>
        </div>
    </div>
