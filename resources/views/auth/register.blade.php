...
<head>
    ...
    <title>{{ __('messages.register') }} - {{ config('app.name', 'DARI') }}</title>
    @if(app()->getLocale() == 'ar')
    ...
    <style>
        ...
        * { transition: all 0.3s ease; box-sizing: border-box; }
        ...
        .input-icon .form-control {
            {{ app()->getLocale() == 'ar' ? 'padding-left' : 'padding-right' }}: 40px;
        }
        .profile-upload {
            text-align: center;
            margin-bottom: 15px;
...
<body>
    <div class="auth-container">
        ...
        <div class="divider"><span>{{ app()->getLocale() == 'ar' ? 'أو أنشئ حساباً جديداً' : 'or create a new account' }}</span></div>
        ...
        <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
            ...
            <div class="profile-upload">
                ...
                <small>{{ app()->getLocale() == 'ar' ? 'الصورة الشخصية (اختياري)' : 'Profile Picture (optional)' }}</small>
            </div>

            <!-- Basic Info -->
            <h6 class="section-title"><i class="bi bi-person me-2"></i>{{ app()->getLocale() == 'ar' ? 'المعلومات الأساسية' : 'Basic Information' }}</h6>
            
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label">{{ __('messages.name') }} <span class="required-star">*</span></label>
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required>
                    @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">{{ app()->getLocale() == 'ar' ? 'اسم المستخدم' : 'Username' }} <span class="required-star">*</span></label>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label">{{ __('messages.email') }} <span class="required-star">*</span></label>
                    <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required>
                    @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">{{ __('messages.phone') }} <span class="required-star">*</span></label>
            ...
            <!-- Location -->
            <h6 class="section-title"><i class="bi bi-geo-alt me-2"></i>{{ app()->getLocale() == 'ar' ? 'الموقع' : 'Location' }}</h6>
            
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label">{{ app()->getLocale() == 'ar' ? 'الدولة' : 'Country' }} <span class="required-star">*</span></label>
                    <select class="form-select @error('country') is-invalid @enderror" name="country" id="countrySelect" required onchange="updateGovernorate()">
                        ...
                        <option value="OM" {{ old('country') == 'OM' ? 'selected' : '' }}>عُمان</option>
                ...
                <div class="col-md-6">
                    <label class="form-label">{{ app()->getLocale() == 'ar' ? 'المحافظة' : 'Governorate' }} <span class="required-star">*</span></label>
                    <select class="form-select @error('governorate') is-invalid @enderror" name="governorate" id="governorateSelect" required disabled>
            ...
            <!-- Password -->
            <h6 class="section-title"><i class="bi bi-lock me-2"></i>{{ __('messages.password') }}</h6>
            
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label">{{ __('messages.password') }} <span class="required-star">*</span></label>
                ...
                <div class="col-md-6">
                    <label class="form-label">{{ __('messages.confirm_password') }} <span class="required-star">*</span></label>
                    <div class="input-icon">
            ...
            <!-- Additional Info -->
            <h6 class="section-title"><i class="bi bi-info-circle me-2"></i>{{ app()->getLocale() == 'ar' ? 'معلومات إضافية' : 'Additional Information' }}</h6>
            
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    <label class="form-label">{{ app()->getLocale() == 'ar' ? 'تاريخ الميلاد' : 'Date of Birth' }}</label>
            ...
            <div class="form-check mb-3">
                ...
                <label class="form-check-label" for="terms">
                    {{ app()->getLocale() == 'ar' ? 'أوافق على' : 'I agree to' }} <a href="#">{{ app()->getLocale() == 'ar' ? 'الشروط والأحكام' : 'Terms and Conditions' }}</a> {{ app()->getLocale() == 'ar' ? 'و' : 'and' }} <a href="#">{{ app()->getLocale() == 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy' }}</a> <span class="required-star">*</span>
                </label>
            </div>
        ...
        <div class="auth-footer">
            {{ app()->getLocale() == 'ar' ? 'لديك حساب بالفعل؟' : 'Already have an account?' }} <a href="{{ route('login') }}">{{ __('messages.login') }}</a>
        </div>
    </div>
